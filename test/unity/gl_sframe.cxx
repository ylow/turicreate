...
struct gl_xframe_test {
  public:
    void test_constructor() {
      gl_xframe sf{{"a", {1,2,3,4}},{"b", {"a","b","c","d"}}};
    ...
    void test_copy() {
      ...
      _assert_xframe_equals(sf, sf3);
      _assert_sarray_equals(sf2["c"], {"x", "x", "x", "x"});
    }

    void test_basic_indexing_and_ranges() {
      gl_xframe sf(_make_reference_frame());
      _assert_flexvec_equals(sf[0], {1, "a"});
      auto res = sf[sf["a"] < 5];
      TS_ASSERT_EQUALS(res.size(), 4);
      _assert_xframe_equals(res, gl_xframe{{"a", {1,2,3,4}},{"b", {"a","b","c","d"}}});
      _assert_xframe_equals(sf[{0, 4}], gl_xframe{{"a", {1,2,3,4}},{"b", {"a","b","c","d"}}});
    ...
    void test_head_and_tail() {
      gl_xframe sf(_make_reference_frame());
      _assert_xframe_equals(sf.head(4), gl_xframe{{"a", {1,2,3,4}},{"b", {"a","b","c","d"}}});
      _assert_xframe_equals(sf.tail(4), gl_xframe{{"a", {7,8,9,10}},{"b", {"g","h","i","j"}}});
    }
    ...
    void test_sample() {
      gl_xframe sf(_make_reference_frame());
      gl_xframe sf2 = sf.sample(0.3);
      TS_ASSERT_LESS_THAN_EQUALS(sf2.size(), sf.size());

      gl_xframe sf3{{"a", {1,2,3,4,5}},
    ...
    void test_sample_split() {
      gl_xframe sf(_make_reference_frame());
      {
        ...
        gl_xframe sfc = sfa.append(sfb);
        _assert_xframe_equals(sf, sfc.sort("a"));
      }
    ...
    void test_groupby() {
      ...
      sf["b"] = 2;
      gl_xframe sf2 = sf.groupby({"a"}, {{"bsum", aggregate::SUM("b")}, {"bcount", aggregate::COUNT()}}).sort("a");
      _assert_xframe_equals(sf2, gl_xframe{{"a", {"a","b"}}, {"bsum", {10, 10}}, {"bcount", {5, 5}}});
    }
    ...
    void test_vector_groupby(){
      ...
      gl_xframe sf2 = sf.groupby({"a"}, {{"bsum", aggregate::SUM("b")}, {"bmean", aggregate::MEAN("b")}}).sort("a");
      _assert_xframe_equals(sf2, gl_xframe{{"a", {"a","b"}}, {"bsum", {{2.0,4.0,6.0},{2.0,4.0,6.0}}}, {"bmean", {{1.0,2.0,3.0},{1.0,2.0,3.0}}}});
    }
    ...
    void test_user_defined_gourpby() {
      ...
      gl_xframe sf2 = sf.groupby({"a"},
                                 {{"blog_sum", aggregate::make_aggregator<log_sum>({"b"})}}).sort("a");
      _assert_xframe_equals(sf2, gl_xframe{{"a", {"a","b"}},
                                           {"blog_sum", {5., 5.}}});
    }

    void test_topk() {
      gl_xframe sf(_make_reference_frame());
      _assert_xframe_equals(sf.topk("b", 4), gl_xframe{{"a", {10,9,8,7}},{"b", {"j","i","h","g"}}});
      _assert_xframe_equals(sf.topk("b", 4, true), gl_xframe{{"a", {1,2,3,4}},{"b", {"a","b","c","d"}}});
    }

    void test_join() {
      gl_xframe sf(_make_reference_frame());
      gl_xframe sf2(_make_reference_frame());

      sf2.rename({{"b", "c"}});
      gl_xframe sf3 = sf.join(sf2, {"a"}, "left");

      sf["c"] = sf["b"];
      _assert_xframe_equals(sf3, sf); 
    }

    void test_pack_unpack() {
      auto reference = _make_reference_frame();
      gl_xframe sf(reference.pack_columns(reference.column_names(), "X1"));
      ...
      _assert_xframe_equals(_make_reference_frame(), sf2);
    }
    ...
    void test_stack_unstack() {
      gl_xframe sf(_make_stacking_frame().unstack("a", "a").sort("b"));
      gl_xframe sf2 = _make_stacking_frame().groupby({"b"}, {{"a", aggregate::CONCAT("a")}}).sort("b");
      ...
      _assert_xframe_equals(sf3, sf4);
    }
    void test_unique() {
      _assert_xframe_equals(_make_reference_frame().unique().sort("a"), _make_reference_frame());
      gl_xframe sf;
      sf["a"] = gl_sarray({1,1,2,2});
      sf["b"] = gl_sarray({"a","a","b","b"});
      _assert_xframe_equals(sf.unique().sort("a"), gl_xframe{{"a",{1,2}},{"b",{"a","b"}}});
    }
    ...
    void test_drop_na() {
      ...
      gl_xframe sf2 = sf.dropna({"a","b"}, "any");
      _assert_xframe_equals(sf2, gl_xframe{{"a",{1,2}},{"b",{"a","b"}}});
      _assert_xframe_equals(sf.dropna({"a","b"}, "all"), sf);
      gl_xframe sf3 = sf.fillna("a", 1).fillna("b","b");
      _assert_xframe_equals(sf3, gl_xframe{{"a",{1,1,2,2}},{"b",{"a","a","b","b"}}});
    }
  ...
  void test_xframe_casts() {
    gl_xframe sf = _make_reference_frame();

    xframe sf2 = sf.materialize_to_xframe();
    gl_xframe sf3 = sf2;

    _assert_xframe_equals(sf, sf3);
  }

  void test_xframe_contains_column() {
    gl_xframe sf = _make_reference_frame();
  ...
  void test_xframe_sort_repeated() {
    // The following code would hit some bug conditions related to cache
    // invalidation.
    gl_xframe sf(make_random_xframe(4, "nnn"));

    for(size_t i = 0; i < 500; ++i) {
      sf["X1-n"] = gl_xframe(make_random_xframe(4, "n"))["X1-n"];
      sf.sort("X1-n");
    }
  }

  void test_xframe_iterator_invalidation() {
    ...
    gl_xframe _make_reference_frame() {
      gl_xframe sf;
    ...
    gl_xframe _make_stacking_frame() {
      gl_xframe sf;
      sf["a"] = gl_sarray::from_sequence(1, 11);
    ...
    std::vector<std::vector<flexible_type> > _to_vec(gl_xframe sa) {
      std::vector<std::vector<flexible_type> > ret;
      for (auto& v: sa.range_iterator()) { ret.push_back(v); }
      return ret;
    }

    void _assert_xframe_equals(gl_xframe sa, gl_xframe sb) {
      TS_ASSERT_EQUALS(sa.size(), sb.size());
...
BOOST_FIXTURE_TEST_SUITE(_gl_xframe_test, gl_xframe_test)
BOOST_AUTO_TEST_CASE(test_constructor) {
  gl_xframe_test::test_constructor();
}
BOOST_AUTO_TEST_CASE(test_copy) {
  gl_xframe_test::test_copy();
}
BOOST_AUTO_TEST_CASE(test_basic_indexing_and_ranges) {
  gl_xframe_test::test_basic_indexing_and_ranges();
}
BOOST_AUTO_TEST_CASE(test_head_and_tail) {
  gl_xframe_test::test_head_and_tail();
}
BOOST_AUTO_TEST_CASE(test_apply) {
  gl_xframe_test::test_apply();
}
BOOST_AUTO_TEST_CASE(test_sample) {
  gl_xframe_test::test_sample();
}
BOOST_AUTO_TEST_CASE(test_sample_split) {
  gl_xframe_test::test_sample_split();
}
BOOST_AUTO_TEST_CASE(test_groupby) {
  gl_xframe_test::test_groupby();
}
BOOST_AUTO_TEST_CASE(test_vector_groupby) {
  gl_xframe_test::test_vector_groupby();
}
BOOST_AUTO_TEST_CASE(test_user_defined_gourpby) {
  gl_xframe_test::test_user_defined_gourpby();
}
BOOST_AUTO_TEST_CASE(test_topk) {
  gl_xframe_test::test_topk();
}
BOOST_AUTO_TEST_CASE(test_join) {
  gl_xframe_test::test_join();
}
BOOST_AUTO_TEST_CASE(test_pack_unpack) {
  gl_xframe_test::test_pack_unpack();
}
BOOST_AUTO_TEST_CASE(test_pack_unpack2) {
  gl_xframe_test::test_pack_unpack2();
}
BOOST_AUTO_TEST_CASE(test_stack_unstack) {
  gl_xframe_test::test_stack_unstack();
}
BOOST_AUTO_TEST_CASE(test_unique) {
  gl_xframe_test::test_unique();
}
BOOST_AUTO_TEST_CASE(test_drop_na) {
  gl_xframe_test::test_drop_na();
}
BOOST_AUTO_TEST_CASE(test_writer) {
  gl_xframe_test::test_writer();
}
BOOST_AUTO_TEST_CASE(test_logical_filter) {
  gl_xframe_test::test_logical_filter();
}
BOOST_AUTO_TEST_CASE(test_filter_by) {
  gl_xframe_test::test_filter_by();
}
BOOST_AUTO_TEST_CASE(test_filter_by_exclude) {
  gl_xframe_test::test_filter_by_exclude();
}
BOOST_AUTO_TEST_CASE(test_save) {
  gl_xframe_test::test_save();
}
BOOST_AUTO_TEST_CASE(test_parallel_range_iterator) {
  gl_xframe_test::test_parallel_range_iterator();
}
BOOST_AUTO_TEST_CASE(test_lazy_iteration) {
  gl_xframe_test::test_lazy_iteration();
}
BOOST_AUTO_TEST_CASE(test_xframe_casts) {
  gl_xframe_test::test_xframe_casts();
}
BOOST_AUTO_TEST_CASE(test_xframe_contains_column) {
  gl_xframe_test::test_xframe_contains_column();
}

BOOST_AUTO_TEST_CASE(test_xframe_sort_repeated) {
  gl_xframe_test::test_xframe_sort_repeated();
}
BOOST_AUTO_TEST_CASE(test_xframe_iterator_invalidation) {
  gl_xframe_test::test_xframe_iterator_invalidation();
}
