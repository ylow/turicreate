...
def _get_default_options_wrapper(
    unity_server_model_name, module_name="", python_class_name="", sdk_model=False
):
    """
    ...
    get_default_options_for_model.__doc__ = get_default_options_for_model.__doc__.format(
        python_class_name=python_class_name, module_name=module_name
    )
...
class Model(ExposeAttributesFromProxy):
    ...
    def _get(self, field):
        ...
        if field in self._list_fields():
            ...
            raise KeyError(
                ('Field "%s" not in model. Available fields are ' "%s.")
    ...
    def summary(self, output=None):
        ...
        if output is None or output == "stdout":
            ...
            raise ToolkitError(
                "Unsupported argument " + str(output) + ' for "summary" parameter.'
            )
...
class CustomModel(ExposeAttributesFromProxy):
    ...
    def name(self):
        ...
        """
        warnings.warn(
            "This function is deprecated. It will be removed in the next release. Please use python's builtin type function instead."
        )
        return self.__class__.__name__
