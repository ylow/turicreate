 ...
 */
#ifndef TURI_XFRAME_QUERY_ENGINE_OPERATORS_QUERY_CONTEXT_HPP
#define TURI_XFRAME_QUERY_ENGINE_OPERATORS_QUERY_CONTEXT_HPP
#include <memory>
#include <vector>
 ...
 *      // fill output buffer. Its just an xframe_rows
...
class query_context {
 public:
   ...
   */
  std::shared_ptr<const xframe_rows> get_next(size_t input_number);
   ...
   */
  std::shared_ptr<xframe_rows> get_output_buffer();
   ...
   */
  void emit(const std::shared_ptr<xframe_rows>& rows);
 ...
 private:
  ...
  // any one point.
  std::shared_ptr<xframe_rows> m_buffers;
...
#endif // TURI_XFRAME_QUERY_ENGINE_OPERATORS_QUERY_CONTEXT_HPP
