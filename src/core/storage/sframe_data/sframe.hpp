 ...
 */
#ifndef TURI_UNITY_LIB_XFRAME_HPP
#define TURI_UNITY_LIB_XFRAME_HPP
#include <iostream>
#include <algorithm>
#include <memory>
#include <vector>
...
typedef turi::xframe_function_output_iterator<
...
/**
 * The XFrame is an immutable object that represents a table with rows
 * and columns.  Each column is an \ref sarray<flexible_type>, which is a
 * sequence of an object T split into segments. The xframe writes an sarray
 * for each column of data it is given to disk, each with a prefix that extends
 * the prefix given to open. The XFrame is referenced on disk by a single
 * ".frame_idx" file which then has a list of file names, one file for each
 * column.
 *
 * The XFrame is \b write-once, \b read-many. The XFrame can be opened for
 * writing \b once, after which it is read-only.
 *
 * Since each column of the XFrame is an independent sarray, as an independent
 * shared_ptr<sarray<flexible_type> > object, columns can be added / removed
 * to form new xframes without problems. As such, certain operations
 ...
 *
 * The interface for the XFrame pretty much matches that of the \ref sarray
 * as in the SArray's stored type is std::vector<flexible_type>. The XFrame
 * however, also provides a large number of other capabilities such as
 * csv parsing, construction from sarrays, etc.
 */
class xframe : public swriter_base<xframe_output_iterator> {
 public:

  /// The reader type
  typedef xframe_reader reader_type;

  /// The iterator type which \ref get_output_iterator returns
  typedef xframe_output_iterator iterator;

  /// The type contained in the xframe
   ...
   * which shares column arrays with the source frame.
   */
  xframe& operator=(const xframe& other);
   ...
   */
  xframe& operator=(xframe&& other);

  /**
   * Attempts to construct an xframe which reads from the given frame
   * index file. This should be a .frame_idx file.
   * If the index cannot be opened, an exception is thrown.
   */
  explicit inline xframe(std::string frame_idx_file) {
    auto frame_index_info = read_xframe_index_file(frame_idx_file);
    open_for_read(frame_index_info);
  }

  /**
   * Construct an xframe from xframe index information.
   */
  explicit inline xframe(xframe_index_file_information frame_index_info) {
    open_for_read(frame_index_info);
  };

  /**
   * Constructs an XFrame from a vector of Sarrays.
   *
   ...
   * sizes, or total sizes of each sarray is not equal. The constructed XFrame
   * is ephemeral, and is not backed by a disk index.
   */
  explicit inline xframe(
  ...
  /**
   * Constructs an XFrame from a csv file.
   *
  ...
  /**************************************************************************/
  /**
   * Initializes the XFrame with an index_information.
   * If the XFrame is already inited, this will throw an exception
   */
  inline void open_for_read(xframe_index_file_information frame_index_info) {
    Dlog_func_entry();
    ASSERT_MSG(!inited, "Attempting to init an XFrame "
  ...
  /**
   * Initializes the XFrame with a collection of columns.  If the XFrame is
  ...
  /**
   * Opens the XFrame with an arbitrary temporary file.
   ...
   * the xframe will throw an exception while constructing the output of
   * this class.
   ...
   * are a part of the xframe. Throws an exception if this is an empty
   * vector.
  ...
  inline void open_for_write(const std::vector<std::string>& column_names,
                             ...
                             size_t nsegments = XFRAME_DEFAULT_NUM_SEGMENTS,
                             bool fail_on_column_names=true) {
    Dlog_func_entry();
    ASSERT_MSG(!inited, "Attempting to init an XFrame "
        "which has already been inited.");
...
/**************************************************************************/
  ...
  /**
   * Return the index file of the xframe
   */
  ...
  /**
   * Reads the value of a key associated with the xframe
  ...
  /**
   * Returns the number of elements in the xframe. If the xframe was not initialized, returns 0.
   */
  ...
  /**
   * Returns the number of segments that this XFrame will be
  ...
  inline size_t segment_length(size_t i) const {
    DASSERT_MSG(inited, "Invalid XFrame");
    if (index_info.ncolumns == 0) return 0;
   ...
   */
  inline const xframe_index_file_information get_index_info() const {
    return index_info;
  }

  /**
   * Merges another XFrame with the same schema with the current XFrame
   * returning a new XFrame.
   * Both XFrames can be empty, but cannot be opened for writing.
   */
  xframe append(const xframe& other) const;


  /**
   * Gets an xframe reader object with the segment layout of the first column.
   */
  std::unique_ptr<reader_type> get_reader() const;


  /**
   * Gets an xframe reader object with num_segments number of logical segments.
   */
  std::unique_ptr<reader_type> get_reader(size_t num_segments) const;


  /**
   * Gets an xframe reader object with a custom segment layout. segment_lengths
...
/**************************************************************************/
  ...
  /**
   * Converts the xframe into a dataframe_t. Will reset iterators before
  ...
  /**
   * Returns new xframe containing only the chosen columns in the same order.
   * The new xframe is "ephemeral" in that it is not backed by an index
   * on disk.
   *
   * Throws an exception if the column name does not exist.
   */
  xframe select_columns(const std::vector<std::string>& names) const;

  /**
   * Returns a new ephemeral XFrame with the new column added to the end.
   * The new xframe is "ephemeral" in that it is not backed by an index
   * on disk.
   *
   ...
   *
   */
  xframe add_column(std::shared_ptr<sarray<flexible_type> > sarr_ptr,
  ...
  /**
   * Returns a new ephemeral XFrame with the column removed.
   * The new xframe is "ephemeral" in that it is not backed by an index
   * on disk.
   *
   ...
   *
   */
  xframe remove_column(size_t column_id) const;


  /**
   * Returns a new ephemeral XFrame with two columns swapped.
   * The new xframe is "ephemeral" in that it is not backed by an index
   * on disk.
   *
   ...
   *
   */
  xframe swap_columns(size_t column_1, size_t column_2) const;

  /**
   * Replace the column of the given column name with a new sarray.
   * Return the new xframe with old column_name sarray replaced by the new sarray.
   */
  xframe replace_column(std::shared_ptr<sarray<flexible_type>> sarr_ptr,
...
// These functions are only valid when the array is opened for writing
   ...
   * behavior.
   *
   * Cannot be called until the xframe is open.
   *
   * Example:
   * \code
   * // example to write the same vector to 7 rows of segment 1
   * // let's say the xframe has 5 columns of type FLEX_TYPE_ENUM::INTEGER
   * // and sfw is the xframe.
  ...
  /**
   * Closes the xframe. close() also implicitly closes all segments.  After
   * the writer is closed, no segments can be written.
   * After the xframe is closed, it becomes read only and can be read
  ...
  /**
   * Saves a copy of the current xframe as a CSV file.
  ...
  /**
   * Saves a copy of the current xframe into a different location.
   * Does not modify the current xframe.
   */
  void save(std::string index_file) const;

  /**
   * XFrame serializer. oarc must be associated with a directory.
  ...
  /**
   * XFrame deserializer. iarc must be associated with a directory.
 ...
 private:
   ...
   * output iterator.  Used by the xframe_output_iterator.
   */
   ...
   * output iterator.  Used by the xframe_output_iterator.
   */
  void write(size_t segmentid, const xframe_rows& t);
   ...
   * the columns array
   */
 void create_arrays_for_reading(xframe_index_file_information frame_index_info);

  /**
   * Internal function. Given a collection of sarray columns, this function
   * makes an xframe representing the combination of all the columns.  This
   * xframe does not have an index file (it is ephemeral), and get_index_file
  ...
  bool inited = false;
  bool writing = false;
  friend class xframe_reader;

public:
  /**
   * For debug purpose, print the information about the xframe.
   */
  void debug_print();
};
