 ...
 */
#ifndef TURI_XFRAME_CONSTANTS_HPP
#define TURI_XFRAME_CONSTANTS_HPP
#include <cstddef>
#include <string>
namespace turi {
...
/**
 * The default number of segments created when an XFrame/SArray is
 * opened for write.
 * (i.e. \ref sarray::open_for_write and \ref xframe::open_for_write).
 * This is default is used in numerous places. For instance the default
 * number of output segments from the xframe_csv_parser, and the dataframe to
 * xframe converter.
 */
extern size_t XFRAME_DEFAULT_NUM_SEGMENTS;
 ...
 * \ref xframe_reader::begin() and \ref xframe_reader::end() also use this
 ...
 * an SArray. (a single column. NOT an xframe).
 */
 ...
 */
extern const size_t XFRAME_WRITER_BUFFER_SOFT_LIMIT;

/**
  * The number of rows to buffer before forcing to flush the buffer to disk.
 * Used in shuffle operation.
 */
extern const size_t XFRAME_WRITER_BUFFER_HARD_LIMIT;

/**
 * The default number of handles in the v2 block manager pool.
 */
extern size_t XFRAME_FILE_HANDLE_POOL_SIZE;


/**
 * The default number of block buffers in the v0 block manager pool.
 */
extern const size_t XFRAME_BLOCK_MANAGER_BLOCK_BUFFER_COUNT;
 ...
 */
extern size_t XFRAME_DEFAULT_BLOCK_SIZE;
 ...
 * XFRAME_DEFAULT_BLOCK_SIZE / (average bytes per element)).
 */
 ...
 * number of elements into a block.
 */
extern size_t XFRAME_WRITER_MAX_BUFFERED_CELLS_PER_BLOCK;
 ...
 * XFRAME_WRITER_MAX_BUFFERED_CELLS_PER_BLOCK to
 * XFRAME_WRITER_MAX_BUFFERED_CELLS / (#columns * #segments)
 */
extern size_t XFRAME_WRITER_MAX_BUFFERED_CELLS;

/**
 * The maximum number of data blocks that can be maintained in a reader's
 * decoded cache
 */
extern size_t XFRAME_MAX_BLOCKS_IN_CACHE;
 ...
 */
extern size_t XFRAME_CSV_PARSER_READ_SIZE;
 ...
 */
extern size_t XFRAME_GROUPBY_BUFFER_NUM_ROWS;

/**
 * The number of elements per bucket when performing a shuffle operation.
 */
extern size_t XFRAME_SHUFFLE_BUCKET_SIZE;

/**
 * The number of bytes that a join algorithm is allowed to use during execution.
 */
extern size_t XFRAME_JOIN_BUFFER_NUM_CELLS;

/**
 * Whether locks are used when reading from XFrames on local storage. Good
 * for spinning disks, bad for SSDs.
 */
extern size_t XFRAME_IO_READ_LOCK;


/**
 * If XFRAME_IO_READ_LOCK is set, then the IO LOCK is only used when the
 * file size is greater than this value.
 */
extern const size_t XFRAME_IO_LOCK_FILE_SIZE_THRESHOLD;

/**
 * Number of samples used to estimate the pivot positions to partition the
 * data for sorting.
 */
extern size_t XFRAME_SORT_PIVOT_ESTIMATION_SAMPLE_SIZE;

/**
 * The maximum number of segments we will try to partition the input XFrame
 * into for external sort.  Number kept low initially to be sensitive of open
 * file handle limits.
 */
extern size_t XFRAME_SORT_MAX_SEGMENTS;

/**
 * The maximum number of segments an XFrame can have after which compaction
 * will be attempted
 */
extern size_t XFRAME_COMPACTION_THRESHOLD;
